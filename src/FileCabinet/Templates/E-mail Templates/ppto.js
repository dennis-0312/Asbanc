        ...
        if (typeMode == 'create' || typeMode == 'copy' || typeMode == 'edit') {
            ...
            if (sublistName == 'line' || sublistName == 'expense' || sublistName == 'item') {
                ...
                if (budgetSearch.custrecord_ts_cp_validate) {
                    ...
                    console.log('Inicio del if : ' + context.currentRecord.type);
                    if (context.currentRecord.type == 'journalentry') {
                        ...
                        console.log('No cumple con ninguna condicón del if');
                        solicitud[partida][line] = parseFloat(transaction.getCurrentSublistValue({ sublistId: sublistName, fieldId: 'amount' }));
                        console.log('Array desde las lineas : ' + solicitud[partida][line]);
                        if (currency != '1') solicitud[partida][line] = parseFloat((solicitud[partida][line] * rate).toFixed(2));
                        console.log('currency : ' + transaction.getText({ fieldId: 'currency' }),);
                        console.log('Array desde las lineas : before  currency' + solicitud[partida][line]);
                    }
                    console.log('Fin del if : ' + context.currentRecord.type);
                    log.error('Trace partida value : ', partida);
                    disponible = getDisponible(partida);

                    // console.log('Analis disponible: ' + disponible);
                    ...
                    console.log("Monto disponible : " + disponible);
                    ...
                    if (disponible >= total) {
                        ...
                        if (typeMode == 'edit') {
                            ...
                            let totalAmount = 0;
                            console.log('Conteo de líneas: ' + countLine);
                            ...
                            if (totalAmount === total) {
                                ...
                                if (Math.abs(disponible) >= totalAmount) {
                                    console.log('Tiene Presupuesto - Dev info 133');
                                } else {
