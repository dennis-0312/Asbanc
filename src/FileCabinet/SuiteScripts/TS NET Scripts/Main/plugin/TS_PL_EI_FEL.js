 ...
 * @NModuleScope Public
 */
define(["N/record", "N/file", "N/email", "N/encode", "N/search", "N/https", "N/log", "N/config"],
    function (record, file, email, encode, search, https, log, config) {
        var recordtype = '';
        var FOLDER_PDF = 1020;          //SB: 421   PR: 1020 - ok
        var FOLDER_EXCEPTIONS = 1019;   //SB: 1403  PR: 1019
        var internalId = '';
        var userId = '';
        ...
        function send(plugInContext) {
            ...
            try {
                ...
                // CUENTA CORRIENTE DE LA COMPAÑIA
                var companyInfo = config.load({ type: config.Type.COMPANY_INFORMATION });
                cta_corriente = companyInfo.getValue('custrecord_il_cuenta_corriente');
        ...
        function createRequest(documentid, array) {
            ...
            try {
                var searchLoad = search.create({
                    ...
                    columns:
                        [
                            ...
                            search.createColumn({
                                name: "formulatext",
                                formula: "CASE WHEN {subsidiary.name} = 'Ibero Librerías' THEN 'Ibero Librerías' END",
                                label: "9 Trade Name"
                            }),
                            search.createColumn({
                                name: "legalname",
                            ...
                            search.createColumn({
                                ...
                                formula: "CASE WHEN {customer.custentity_pe_document_type} = 'Registro Unico De Contribuyentes' THEN '6' WHEN {customer.custentity_pe_document_type} = 'Documento Nacional De Identidad (DNI)' THEN '1' WHEN {customer.custentity_pe_document_type} = 'Otros Tipos De Documentos' THEN '0' END",
                            ...
                            // IL NÚMERO DE VUELO --------------------------------------------------------------------------------------------------------
                            search.createColumn({ name: "custbody_il_numero_vuelo", label: "48 IL NÚMERO VUELO" }),
                            // IL PASAPORTE --------------------------------------------------------------------------------------------------------------
                            search.createColumn({ name: "custbody_il_pasaporte", label: "49 IL PASAPORTE" }),
                            // IL PAÍS ----------------------------------------------------------------------------------------------------------------
                            search.createColumn({ name: "custbody_il_pais", label: "50 IL PAÍS" }),
                            // IL NOMBRE --------------------------------------------------------------------------------------------------------------
                            search.createColumn({ name: "custbody_il_nombre", label: "51 IL NOMBRE" }),
                            ...
                            // IL NOTA
                            search.createColumn({ name: "memo", label: "54 IL MEMO" })
                        ]
                });
                var searchResult = searchLoad.run().getRange({ start: 0, end: 1 });
                // IDE---------------------------------------------------------------------------------------------------------------------
                ...
                var id_location = searchResult[0].getValue(searchLoad.columns[42]); // Ibero Aeropuerto

                var nmro_documento = '';
                var razon_social = '';
                var direccion_cliente = '';

                if (id_location == IBERO_AEROPUERTO) {
                    // IL NÚMERO DE VUELO --------------------------------------------------------------------------------------------------------
                    var column48 = searchResult[0].getValue(searchLoad.columns[47]);
                    // IL PASAPORTE --------------------------------------------------------------------------------------------------------------
                    var column49 = searchResult[0].getValue(searchLoad.columns[48]);
                    // IL PAÍS ----------------------------------------------------------------------------------------------------------------
                    var column50 = searchResult[0].getText(searchLoad.columns[49]);
                    // IL NOMBRE --------------------------------------------------------------------------------------------------------------
                    var column51 = searchResult[0].getValue(searchLoad.columns[50]);

                    nmro_documento = column21;
                    razon_social = column51 + ' ' + column48 + ' ' + column49;
                    direccion_cliente = column50;
                ...
                var detail = getDetail(documentid, array, column47, objPromocion);
                ...
                if (column42 != '') fulfillment = openFulfillment(column42);
                ...
                jsonADI2 = [
                    {
                        tituloAdicional: "CUENTAS CORRIENTES",
                        valorAdicional: cta_corriente
                    },
                    {
                        tituloAdicional: "NOTA",
                        valorAdicional: column54
                    }
                ]
        ...
        function getDetail(documentid, array, freeop, _objPromocion) {
            var json = new Array();
            ...
            try {
                ...
                var codigocargodescuento_prom = taxcode_display_prom == 'IGV_PE:S-PE' ? "02" : "03";
                ...
                var searchLoad = search.create({
                    ...
                    columns:
                        [
                            ...
                            search.createColumn({ name: "billcountrycode", label: "15 Billing Country Code" }),
                            search.createColumn({
                                name: "phone",
                                join: "subsidiary",
                                label: "16 Phone"
                            ...
                            // IL NOTA
                            search.createColumn({ name: "memo", label: "49 IL MEMO" })
                        ]
                });
