 ...
 * GAP Int       14 Ago 2023     Giovana Guadalupe <giovana.guadalupe@myevol.biz>     - CP validacion Telefonica - Paso x CP
 *
 */
define(["N/search", "N/record", "N/log", "N/file", "N/task", "xlsx", "../Library/TS_LIB_ControlPresupuestal.js", "N/https", "N/runtime"], (search, record, log, file, task, XLSX, libCP, https, runtime) => {
  const execute = (context) => {
        ...
        if (!flagItem) {
          ...
          if (validCP.value) {
            ...
            if (remainingUsage2 <= 5000) {
              ...
              var params = {
                custscript_ts_ss_load_tf_data_file: paramIDFile,
                custscript_ts_ss_load_tf_data_cont: paramCont,
                custscript_ts_ss_load_tf_data_json: idFileJsonAux,
                custscript_ts_ss_load_tf_data_info: JSON.stringify(paramArchivoInfo),
              };
              var scriptTask = task.create({
                taskType: task.TaskType.SCHEDULED_SCRIPT,
                scriptId: runtime.getCurrentScript().id,
    ...
    if (executionCompleted) {
      ...
      var paramsCreate = {
        custscript_ts_ss_create_tf_file: idFileJsonAux,
        custscript_ts_ss_create_tf_cont: 0,
        custscript_ts_ss_create_tf_data_file: paramArchivoInfo,
      };

      // log.debug("paramsCreate", paramsCreate);
      var scriptCreateTask = task.create({
        taskType: task.TaskType.SCHEDULED_SCRIPT,
        scriptId: "customscript_ts_ss_createtelefonicadata",
        deploymentId: "customdeploy_ts_ss_createtelefonicadata",
        params: paramsCreate,
      });
