 ...
 * GAP Int Claro      14 Ago 2023     Giovana Guadalupe <giovana.guadalupe@myevol.biz>      Validacion y tipo de cambio
 *
 */
define(["N/search", "N/record", "N/log", "N/file", "N/task", "xlsx", "../Library/TS_LIB_ControlPresupuestal.js", "N/https", "N/runtime"], (search, record, log, file, task, XLSX, libCP, https, runtime) => {
  const execute = (context) => {
    const FN = "execute";
    var aprobadorServiciosClaro = 60;//IVAN ESTRADA iestrada@asbanc.com.pe
    ...
    try {
      ...
      if (!archivo) {
        ...
        if (excelFile && excelFile.fileType === "EXCEL") {
          ...
          let fechaEmisionLineas = rows[0].indexOf("FECHA EMISION"); // > TELEFONICA
          let fechaEmisionServicios = rows[0].indexOf("fecemi");// > claro
          ...
          if (fechaEmisionServicios > -1) {
            //Servicios Claro
            log.debug("validando", "archivo de servicios claro");
            ...
            for (let i = 1; i < rows.length; i++) {
              ...
              if (serie) {
                if (fecemiDate) {
                  ...
                  // actualización para tipo de impuesto
                  // let impuestoValue;
                  // if (impuesto.includes("IGV")){
                  //   impuestoValue = 6
                  // } else {
                  ...
                  var header = {
                    ...
                    aprobador: aprobadorServiciosClaro,
                  };
          ...
          if (!flagInvalid && !flagMoneda && Object.keys(newData).length > 0) {
            ...
            var params = {
              custscript_ts_ss_load_claro_data_file: idFile,
              custscript_ts_ss_load_claro_data_cont: 0,
              custscript_ts_ss_load_claro_data_json: 0,
              custscript_ts_ss_load_claro_data_info: archivo,
            };

            var redirectSchdl = task.create({
              taskType: task.TaskType.SCHEDULED_SCRIPT,
              scriptId: "customscript_ts_ss_load_claro_data",
              deploymentId: "customdeploy_ts_ss_load_claro_data",
              params: params,
            });
  ...
  const searchFileCabinet = () => {
    ...
    try {
      let searchObj = search.create({
        type: "customrecord_archivos_sftp",
        filters: [["custrecord_ns_tipo", "anyof", "3"], "AND", ["custrecord_ns_estado", "anyof", "1"], "AND", ["custrecord_nombre_archivo", "contains", ".xlsx"]],
        columns: [
          search.createColumn({
  ...
  //MEJORA PARA CONSULTAR TIPO UNA VEZ AL DÍA Y GUARDAR EN CUSTOM RECORD
