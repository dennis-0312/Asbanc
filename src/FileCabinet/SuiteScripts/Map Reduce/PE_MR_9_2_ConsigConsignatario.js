    ...
    (runtime, search, config, render, record, file, libPe) => {
        ...
        const getJsonData = (transactions) => {
            ...
            for (let k in transactions) {
                ...
                let IDD = transactions[k].fechaRecepcion;
                jsonTransacion[IDD] = {
        ...
        const getTemplate = () => {
            var aux = file.load("../FTL/PE_Template9_2ConsigConsignatario.ftl");
        ...
        const getTransactions = () => {
            ...
            var _cont = 0;
            // PE - Libro Diario 9.2
            var savedSearch = search.load({ id: 'customsearch_asb_9_1_libro_impreso_2' });
            ...
            pagedData.pageRanges.forEach(function (pageRange) {
                ...
                page.data.forEach((result) => {
                    ...
                    arrAux[3] = result.getValue(columns[4]);

                    // 6. NÚMERO DE GUÍA DE REMISIÓN
                    arrAux[4] = result.getValue(columns[5]);

                    // 7. SERIE DE COMPROBANTE DE PAGO
                    arrAux[5] = result.getValue(columns[6]);

                    // 8. NÚMERO DE COMPROBANTE DE PAGO
                    arrAux[6] = result.getValue(columns[7]);

                    // 9.RUC
                    arrAux[9] = result.getValue(columns[8]);

                    // 10. RAZÓN SOCIAL
                    arrAux[10] = result.getValue(columns[9]);

                    // 11. SUM OF CANTIDAD ENTREGADA
                    arrAux[11] = result.getValue(columns[10]);

                    // 12. SUM OF CANTIDAD DEVUELTA
                    arrAux[12] = result.getValue(columns[11]);

                    // 13. SUM OF CANTIDAD VENDIDA
                    arrAux[13] = result.getValue(columns[12]);

                    // 14. SUM OF SALDO DE LOS BIENES EN CONSIGNACIÓN
                    arrAux[14] = result.getValue(columns[13]);
