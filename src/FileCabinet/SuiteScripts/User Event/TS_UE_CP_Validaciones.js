                ...
                if (context.type === 'view') {
                    if (statusTransf == '1' && request != '4') {
                        if (transfAprobador == sessionObj) {
                            ...
                            let btnRechazar = form.addButton({
                    ...
                    let formSublist = form.getSublist({
                        id: 'recmachcustrecord_ts_cp_item_transfers'
                    });
                    ...
                    if (statusTransf == '1') {
                        if (transfAprobador == sessionObj) {
                            let btnAprobar = form.addButton({
                                id: 'custpage_ts_btn_aprobar',
                                label: "Aprobar",
                                functionName: 'statusAprobacionMasiv(' + currentRecord.id + ',"' + currentRecord.type + '", 2,' + transferencia + ')'
                            });

                            let btnRechazar = form.addButton({
                                id: 'custpage_ts_btn_rechazar',
                                label: "Rechazar",
                                functionName: 'statusAprobacionMasiv(' + currentRecord.id + ',"' + currentRecord.type + '", 3,' + transferencia + ')'
                            });
                        }
                    } else if (statusTransf == '2') {
                        var editTransfer = form.getButton('edit');
